...
class ConversationStore:
    ...
    def append_message(self, thread_id: str, role: str, text: str) -> int:
        ...
        return self.conn.execute("SELECT seq FROM message WHERE id = ?", (cursor.lastrowid,)).fetchone()[0]
        ...
        with self.transaction() as conn:
            ...
            call_id = conn.execute(
                """INSERT INTO tool_call (run_step_id, tool_name, args, result, ok, latency_ms)
                   VALUES (?, ?, ?, ?, ?, ?)""",
                (step_id, name, json.dumps(args), json.dumps(result), int(ok), latency_ms)).lastrowid
            return step_id
